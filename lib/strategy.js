...
console.log('facebook url: ' + url);
  ...
  this._oauth2.get(url, accessToken, function (err, body, res) {
    ...
    var friendsUrl = url + '/friends?fields=username,name,gender';
    that._oauth2.get(friendsUrl, accessToken, function(err, body,res) {
        if (!err) {
            try {
                json = JSON.parse(body);
            } catch(ex) {
            ...
            profile.contacts = json.data;
        } else {
            console.log('get facebook friends error: ' + err);
        }
